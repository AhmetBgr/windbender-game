...
public class ChangeWindRoute : Command{
    ...
    public ChangeWindRoute(GameManager gameManager, List<WindCutRequest> cutRequests, List<WindRestoreRequest> restoreRequests) {
        ...
        this.oldRoute.AddRange(gameManager.curGame.route);
    ...
    public override void Execute() {
        ...
        if (windCutRequest.windCutter != null && windCutRequest.cutIndex < windRestoreRequest.index) {
            ...
            windCutRequest.windCutter.CutWind(gameManager.curGame.route, windCutRequest.cutIndex);

            //gameManager.windCutRequests.Clear();
            gameManager.curGame.curWindCutRequest = windCutRequest;
            Debug.Log("cur wint cutter name: " + gameManager.curGame.curWindCutRequest.windCutter.name);
        }
        ...
        if (windRestoreRequest.windCutter != null) {
            ...
            int cutLenght = gameManager.curGame.curWindSource.defWindSP - windRestoreRequest.index - 1;
            windRestoreRequest.windCutter.Reflect(gameManager.curGame.route, windRestoreRequest.restoreDir, cutLenght, windRestoreRequest.index, false);

            //gameManager.windRestoreRequests.Clear();
        }

        // draw wind route
        if (gameManager.curGame.route.Count == 0) return;
    ...
    public override void Undo() {
        ...
        gameManager.curGame.route.AddRange(oldRoute);
        gameManager.wind.DrawWind();
